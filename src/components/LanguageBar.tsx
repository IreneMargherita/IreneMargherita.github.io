...
export default function LanguageBar({ languages }: { languages: ProjectLanguage[] }) {
  return (
    <div>
      <div className="flex h-2 w-full overflow-hidden rounded-full bg-surface" aria-hidden="true">
        {languages.map((l) => (
      ...
      <ul className="mt-2 flex flex-wrap gap-x-4 gap-y-1">
        {languages.map((l) => (
          <li key={l.name} className="flex items-center gap-1.5 font-mono text-[11px] text-fg-faint">
            <span className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: l.color }} aria-hidden="true" />
            {l.name} <span className="text-fg-faint">{l.pct}%</span>
          </li>
        ))}
      </ul>
    </div>
